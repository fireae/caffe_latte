  ...
  for (int transitionIdx = 0; transitionIdx < this->numTransition;
    ...
    if (!useBC) {
      ...
      shared_ptr<Filler<Dtype> > filter_Filler(
          GetFiller<Dtype>(dbParam.filter_filler()));
      ...
      shared_ptr<Filler<Dtype> > filter_Filler3(
          GetFiller<Dtype>(dbParam.filter_filler()));
      ...
      this->blobs_[5 * numTransition + transitionIdx].reset(
          new Blob<Dtype>(filter11Shape));
      shared_ptr<Filler<Dtype> > filter_Filler1(
          GetFiller<Dtype>(dbParam.filter_filler()));
      filter_Filler1->Fill(
    ...
    // scaler
    this->blobs_[numTransition + transitionIdx].reset(
        new Blob<Dtype>(BNparamShape));
    shared_ptr<Filler<Dtype> > weight_filler0(
        GetFiller<Dtype>(dbParam.bn_scaler_filler()));
    ...
    if (useBC) {
      this->blobs_[6 * numTransition + transitionIdx].reset(
          new Blob<Dtype>(BN_4Gparam_ShapeVec));
      shared_ptr<Filler<Dtype> > weight_filler0_4G(
          GetFiller<Dtype>(dbParam.bn_scaler_filler()));
    ...
    this->blobs_[2 * numTransition + transitionIdx].reset(
        new Blob<Dtype>(BNparamShape));
    shared_ptr<Filler<Dtype> > weight_filler1(
        GetFiller<Dtype>(dbParam.bn_bias_filler()));
    weight_filler1->Fill(this->blobs_[2 * numTransition + transitionIdx].get());
    // bias BC
    if (useBC) {
      this->blobs_[7 * numTransition + transitionIdx].reset(
          new Blob<Dtype>(BN_4Gparam_ShapeVec));
      shared_ptr<Filler<Dtype> > weight_filler1_4G(
          GetFiller<Dtype>(dbParam.bn_bias_filler()));
    ...
    for (int blobIdx = 0; blobIdx < inChannels; ++blobIdx) {
      shared_ptr<Blob<Dtype> > localB =
          this->blobs_[3 * numTransition + transitionIdx];
    ...
    if (useBC) {
      this->blobs_[8 * numTransition + transitionIdx].reset(
          new Blob<Dtype>(BN_4Gparam_ShapeVec));
      shared_ptr<Blob<Dtype> > localB =
          this->blobs_[8 * numTransition + transitionIdx];
    ...
    for (int blobIdx = 0; blobIdx < inChannels; ++blobIdx) {
      shared_ptr<Blob<Dtype> > localB =
          this->blobs_[4 * numTransition + transitionIdx];
    ...
    if (useBC) {
      this->blobs_[9 * numTransition + transitionIdx].reset(
          new Blob<Dtype>(BN_4Gparam_ShapeVec));
      shared_ptr<Blob<Dtype> > localB =
          this->blobs_[9 * numTransition + transitionIdx];
      for (int blobIdx = 0; blobIdx < 4 * growthRate; ++blobIdx) {
        localB->mutable_cpu_data()[localB->offset(0, blobIdx, 0, 0)] = 1;
      }
    }
...
Dtype getZeroPaddedValue(bool isDiff, Blob<Dtype>* inputData, int n, int c,
  ...
  if (isDiff)
    return inputData->diff_at(n, c, h, w);
  else
...
// CPU version of convolution assume img H,W does not change after convolution,
// which corresponds to denseBlock without BC
// input of size N*c_input*h_img*w_img
template <typename Dtype>
void convolution_Fwd(Blob<Dtype>* input, Blob<Dtype>* output,
                     Blob<Dtype>* filter, int N, int c_output, int c_input,
                     int h_img, int w_img, int h_filter, int w_filter) {
...
template <typename Dtype>
void convolution_Bwd(Blob<Dtype>* bottom, Blob<Dtype>* top, Blob<Dtype>* filter,
                     int N, int c_output, int c_input, int h_img, int w_img,
          ...
          for (int n = 0; n < N; ++n) {
            for (int i_img = 0; i_img < h_img; ++i_img) {
              for (int j_img = 0; j_img < w_img; ++j_img) {
                ...
                localGradSum += top->diff_at(n, coutIdx, i_img, j_img) *
                                getZeroPaddedValue(false, bottom, n, cinIdx,
                                                   localX, localY);
              }
            }
          }
          filterDiffPtr[filter->offset(coutIdx, cinIdx, filter_x, filter_y)] =
              localGradSum;
        }
      }
    }
          ...
          for (int coutIdx = 0; coutIdx < c_output; ++coutIdx) {
            for (int x_img = 0; x_img < h_img; ++x_img) {
              for (int y_img = 0; y_img < w_img; ++y_img) {
                ...
                localGradSum += top->diff_at(n, coutIdx, x_img, y_img) *
                                getZeroPaddedValue(false, filter, coutIdx,
                                                   cinIdx, localX, localY);
              }
            }
          }
...
template <typename Dtype>
void ReLU_Fwd(Blob<Dtype>* bottom, Blob<Dtype>* top, int N, int C, int h_img,
              int w_img) {
  // Reshape top
...
template <typename Dtype>
void ReLU_Bwd(Blob<Dtype>* bottom, Blob<Dtype>* top, int N, int C, int h_img,
              int w_img) {
  Dtype* bottomDiffPtr = bottom->mutable_cpu_diff();
  for (int n = 0; n < N; ++n) {
    for (int cIdx = 0; cIdx < C; ++cIdx) {
      for (int hIdx = 0; hIdx < h_img; ++hIdx) {
        for (int wIdx = 0; wIdx < w_img; ++wIdx) {
          bottomDiffPtr[bottom->offset(n, cIdx, hIdx, wIdx)] =
              bottom->data_at(n, cIdx, hIdx, wIdx) >= 0
                  ? top->diff_at(n, cIdx, hIdx, wIdx)
                  : 0;
        }
      }
    }
...
template <typename Dtype>
void BN_inf_Fwd(Blob<Dtype>* input, Blob<Dtype>* output, int N, int C,
                int h_img, int w_img, Blob<Dtype>* globalMean,
                Blob<Dtype>* globalVar, Blob<Dtype>* scaler, Blob<Dtype>* bias,
                Blob<Dtype>* factor_b) {
...
template <typename Dtype>
void BN_train_Fwd(Blob<Dtype>* bottom, Blob<Dtype>* top,
                  Blob<Dtype>* output_xhat, Blob<Dtype>* globalMean,
                  Blob<Dtype>* globalVar, Blob<Dtype>* batchMean,
                  Blob<Dtype>* batchVar, Blob<Dtype>* scaler, Blob<Dtype>* bias,
                  int N, int C, int h_img, int w_img, Dtype EMA_decay) {
  // reshape output
  int outputShape[] = {N, C, h_img, w_img};
  vector<int> outputShapeVec(outputShape, outputShape + 4);
...
template <typename Dtype>
bool decide_channelDiffAllZero(Blob<Dtype>* B, int channelIdx, int N, int C,
                               int H, int W) {
  bool output = true;
  for (int n = 0; n < N; ++n) {
    for (int h = 0; h < H; ++h) {
      for (int w = 0; w < W; ++w) {
        output = output && (B->diff_at(n, channelIdx, h, w) < 0.001) &&
                 (B->diff_at(n, channelIdx, h, w) > -0.001);
      }
    }
  }
  return output;
...
template <typename Dtype>
void BN_train_Bwd(Blob<Dtype>* bottom, Blob<Dtype>* bottom_xhat,
                  Blob<Dtype>* top, Blob<Dtype>* batchMean,
                  Blob<Dtype>* batchVar, Blob<Dtype>* scaler, Blob<Dtype>* bias,
                  int N, int C, int h_img, int w_img, bool betaOneData) {
  double epsilon = 1e-5;
    ...
    for (int n = 0; n < N; ++n) {
      for (int hIdx = 0; hIdx < h_img; ++hIdx) {
        for (int wIdx = 0; wIdx < w_img; ++wIdx) {
          ...
          scalerGrad[channelIdx] +=
              top->diff_at(n, channelIdx, hIdx, wIdx) *
              bottom_xhat->data_at(n, channelIdx, hIdx, wIdx);
        }
      }
    }
  }
  ...
  for (int n = 0; n < N; ++n) {
    for (int c = 0; c < C; ++c) {
      for (int h = 0; h < h_img; ++h) {
        for (int w = 0; w < w_img; ++w) {
          XhatGrad[bottom_xhat->offset(n, c, h, w)] =
              top->diff_at(n, c, h, w) * scaler->data_at(0, c, 0, 0);
        }
      }
    }
  }
  ...
  for (int c = 0; c < C; ++c) {
    for (int n = 0; n < N; ++n) {
      for (int h = 0; h < h_img; ++h) {
        for (int w = 0; w < w_img; ++w) {
          // varGrad[c] += bottom_xhat->diff_at(n,c,h,w) *
          // (bottom->data_at(n,c,h,w)-batchMean->data_at(0,c,0,0)) * (-0.5) *
          // pow(batchVar->data_at(0,c,0,0) + epsilon,-1.5);
          varGrad[c] +=
    ...
    for (int n = 0; n < N; ++n) {
      for (int h = 0; h < h_img; ++h) {
        for (int w = 0; w < w_img; ++w) {
          meanGrad[c] +=
              bottom_xhat->diff_at(n, c, h, w) *
                  (-1.0 / sqrt(batchVar->data_at(0, c, 0, 0) + epsilon)) +
              batchVar->diff_at(0, c, 0, 0) * (-2.0) *
                  (bottom->data_at(n, c, h, w) -
                   batchMean->data_at(0, c, 0, 0)) /
                  m;
          // if (decide_channelDiffAllZero<Dtype>(top,c,N,C,h_img,w_img)){
          //  std::cout<<varGrad[c]<<std::endl;
          //}
  ...
  for (int n = 0; n < N; ++n) {
    for (int c = 0; c < C; ++c) {
      for (int h = 0; h < h_img; ++h) {
        for (int w = 0; w < w_img; ++w) {
          // Dtype
          // term1=bottom_xhat->diff_at(n,c,h,w)*pow(batchVar->data_at(0,c,0,0)+epsilon,-0.5);
          Dtype term1 = bottom_xhat->diff_at(n, c, h, w) /
                        (sqrt(batchVar->data_at(0, c, 0, 0) + epsilon));
          Dtype term2 =
              batchVar->diff_at(0, c, 0, 0) * 2.0 *
              (bottom->data_at(n, c, h, w) - batchMean->data_at(0, c, 0, 0)) /
...
void DenseBlockLayer<Dtype>::CPU_Initialization() {
  ...
  this->merged_conv[this->numTransition] =
      new Blob<Dtype>(extraMergeOutputShapeVector);
}

template <typename Dtype>
void mergeChannelData(Blob<Dtype>* outputBlob, Blob<Dtype>* blobA,
                      Blob<Dtype>* blobB) {
  int N = blobA->shape(0);
...
template <typename Dtype>
void distributeChannelDiff(Blob<Dtype>* inputBlob, Blob<Dtype>* blobA,
                           Blob<Dtype>* blobB) {
  int N = blobA->shape(0);
  int frontC = blobA->shape(1);
  int backC = blobB->shape(1);
      ...
      for (int h = 0; h < H; ++h) {
        for (int w = 0; w < W; ++w) {
          ...
          if (c < frontC) {
            ...
            int writeC = c - frontC;
            blobB->mutable_cpu_diff()[blobB->offset(n, writeC, h, w)] =
                readData;
          }
        }
      }
...
void DenseBlockLayer<Dtype>::Forward_cpu(const vector<Blob<Dtype>*>& bottom,
  ...
  for (int transitionIdx = 0; transitionIdx < this->numTransition;
    ...
    if (this->phase_ == TEST) {
      ...
      BN_inf_Fwd<Dtype>(
          BN_bottom, BN_top, this->N, localChannels, this->H, this->W,
          this->blobs_[3 * this->numTransition + transitionIdx].get(),
          this->blobs_[4 * this->numTransition + transitionIdx].get(), Scaler,
          Bias, this->blobs_[bnTimerIdx].get());
    } else {
      // std::cout<<"cpu BN train forward"<<std::endl;
      BN_train_Fwd<Dtype>(
          BN_bottom, BN_top, this->BN_XhatVec[transitionIdx],
          this->blobs_[3 * this->numTransition + transitionIdx].get(),
          this->blobs_[4 * this->numTransition + transitionIdx].get(),
          this->batch_Mean[transitionIdx], this->batch_Var[transitionIdx],
          Scaler, Bias, this->N, localChannels, this->H, this->W,
          this->EMA_decay);
    }
    // ReLU
    ...
    if (useBC) {
      ...
      convolution_Fwd<Dtype>(BC_conv_x, BC_conv_y, BC_filterBlob, N,
      ...
      Blob<Dtype>* BC_Scaler =
          this->blobs_[6 * numTransition + transitionIdx].get();
      Blob<Dtype>* BC_Bias =
          this->blobs_[7 * numTransition + transitionIdx].get();
      Blob<Dtype>* BC_Mean =
          this->blobs_[8 * numTransition + transitionIdx].get();
      Blob<Dtype>* BC_Var =
          this->blobs_[9 * numTransition + transitionIdx].get();
      if (this->phase_ == TEST) {
        BN_inf_Fwd<Dtype>(BC_BN_x, BC_BN_y, N, 4 * growthRate, H, W, BC_Mean,
                          BC_Var, BC_Scaler, BC_Bias,
                          this->blobs_[bnTimerIdx].get());
      } else {
        ...
        BN_train_Fwd<Dtype>(BC_BN_x, BC_BN_y, BC_xhat, BC_Mean, BC_Var,
                            BC_batchMean, BC_batchVar, BC_Scaler, BC_Bias, N,
                            4 * growthRate, H, W, EMA_decay);
      }
      // BC ReLU
    ...
    convolution_Fwd<Dtype>(conv_x, conv_y, filterBlob, N, growthRate,
                           inConvChannel, H, W, 3, 3);
    // post Conv merge
...
void DenseBlockLayer<Dtype>::Backward_cpu(const vector<Blob<Dtype>*>& top,
  ...
  for (int transitionIdx = this->numTransition - 1; transitionIdx >= 0;
       --transitionIdx) {
    // distribute diff
    distributeChannelDiff(this->merged_conv[transitionIdx + 1],
                          this->merged_conv[transitionIdx],
                          this->postConv_blobVec[transitionIdx]);
    // Conv Bwd
    ...
    int c_input =
        useBC ? 4 * growthRate : initChannel + growthRate * transitionIdx;
    convolution_Bwd<Dtype>(conv_bottom, conv_top, filter, this->N,
                           this->growthRate, c_input, this->H, this->W, 3, 3);
    ...
    if (useBC) {
      ...
      Blob<Dtype>* BC_batchMean = batch_Mean4G[transitionIdx];
      Blob<Dtype>* BC_batchVar = batch_Var4G[transitionIdx];
      BN_train_Bwd<Dtype>(BC_BN_x, BC_BN_xhat, BC_BN_y, BC_batchMean,
                          BC_batchVar, BC_Scaler, BC_Bias, N, 4 * growthRate, H,
                          W, false);
      // Conv1*1 BC Bwd
      Blob<Dtype>* BC_conv_x = postReLU_blobVec[transitionIdx];
      Blob<Dtype>* BC_conv_y = postConv_BCVec[transitionIdx];
      Blob<Dtype>* BC_filter =
          this->blobs_[5 * numTransition + transitionIdx].get();
      int BC_c_input = initChannel + growthRate * transitionIdx;
      int BC_c_output = 4 * growthRate;
      convolution_Bwd<Dtype>(BC_conv_x, BC_conv_y, BC_filter, N, BC_c_output,
                             BC_c_input, H, W, 1, 1);
    }
    // ReLU Bwd
    int localChannel = this->initChannel + this->growthRate * transitionIdx;
    ReLU_Bwd<Dtype>(postBN_blobVec[transitionIdx],
                    postReLU_blobVec[transitionIdx], this->N, localChannel,
                    this->H, this->W);
    // BN Bwd
    Blob<Dtype>* BN_bottom = this->merged_conv[transitionIdx];
    Blob<Dtype>* scaler =
        this->blobs_[this->numTransition + transitionIdx].get();
    Blob<Dtype>* bias =
        this->blobs_[2 * this->numTransition + transitionIdx].get();
    BN_train_Bwd<Dtype>(BN_bottom, this->BN_XhatVec[transitionIdx],
                        this->postBN_blobVec[transitionIdx],
                        this->batch_Mean[transitionIdx],
                        this->batch_Var[transitionIdx], scaler, bias, this->N,
                        localChannel, this->H, this->W, true);
  }
...
template <typename Dtype>
void DenseBlockLayer<Dtype>::Forward_cpu_public(
    const vector<Blob<Dtype>*>& bottom, const vector<Blob<Dtype>*>& top) {
  this->Forward_cpu(bottom, top);
}

template <typename Dtype>
void DenseBlockLayer<Dtype>::Backward_cpu_public(
