...
void ROIWarpingLayer<Dtype>::LayerSetUp(const vector<Blob<Dtype>*>& bottom,
  ...
  ROIWarpingParameter roi_warping_param =
      this->layer_param_.roi_warping_param();
  CHECK_GT(roi_warping_param.pooled_h(), 0) << "pooled_h must be > 0";
  CHECK_GT(roi_warping_param.pooled_w(), 0) << "pooled_w must be > 0";
...
template <typename Dtype>
void ROIWarpingLayer<Dtype>::Reshape(const vector<Blob<Dtype>*>& bottom,
                                     const vector<Blob<Dtype>*>& top) {
  ...
  max_idx_h_.Reshape(bottom[1]->num(), channels_, pooled_height_,
                     pooled_width_);
  buffer_.Reshape(bottom[1]->num() * 5, channels_, pooled_height_,
                  pooled_width_);
}
