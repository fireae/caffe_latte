  ...
  inline bool LoadwithDifficult(std::ifstream& infile) {
    ...
    for (int i = 0; i < num_roi; ++i) {
      ...
      CHECK(infile >> label >> x1 >> y1 >> x2 >> y2 >> diff_)
          << "illegal line of " << image_path;
      // CHECK(label > 0 && label < FrcnnParam::n_classes) << "illegal label: "
      // << label << ", should >= 1 and < "<< FrcnnParam::n_classes;
      CHECK_GT(x2, x1) << "illegal coordinate : " << x1 << ", " << x2 << " : "
                       << this->image_path;
      CHECK_GT(y2, y1) << "illegal coordinate : " << y1 << ", " << y2 << " : "
                       << this->image_path;
