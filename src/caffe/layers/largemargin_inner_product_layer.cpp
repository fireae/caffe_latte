...
void LargeMarginInnerProductLayer<Dtype>::LayerSetUp(
  ...
  const int num_output =
      this->layer_param_.largemargin_inner_product_param().num_output();
  N_ = num_output;
...
template <typename Dtype>
void LargeMarginInnerProductLayer<Dtype>::Reshape(
    const vector<Blob<Dtype>*>& bottom, const vector<Blob<Dtype>*>& top) {
...
template <typename Dtype>
void LargeMarginInnerProductLayer<Dtype>::Forward_cpu(
    const vector<Blob<Dtype>*>& bottom, const vector<Blob<Dtype>*>& top) {
  iter_ += (Dtype)1.;
  ...
  Dtype lambda_min_ =
      this->layer_param_.largemargin_inner_product_param().lambda_min();
  ...
  for (int i = 0; i < M_; i++) {
    mutable_x_norm_data[i] =
        sqrt(caffe_cpu_dot(K_, bottom_data + i * K_, bottom_data + i * K_));
  }
  const Dtype* weight = this->blobs_[0]->cpu_data();
  Dtype* mutable_w_norm_data = w_norm_.mutable_cpu_data();
  for (int i = 0; i < N_; i++) {
    mutable_w_norm_data[i] =
        sqrt(caffe_cpu_dot(K_, weight + i * K_, weight + i * K_));
  }
  ...
  caffe_cpu_gemm<Dtype>(CblasTrans, CblasNoTrans, M_, N_, 1, (Dtype)1.,
                        x_norm_.cpu_data(), w_norm_.cpu_data(), (Dtype)0.,
                        xw_norm_product_.mutable_cpu_data());
  caffe_cpu_gemm<Dtype>(CblasNoTrans, CblasTrans, M_, N_, K_, (Dtype)1.,
                        bottom_data, weight, (Dtype)0.,
                        cos_theta_.mutable_cpu_data());
  caffe_add_scalar(M_ * N_, (Dtype)0.000000001,
                   xw_norm_product_.mutable_cpu_data());
  caffe_div(M_ * N_, cos_theta_.cpu_data(), xw_norm_product_.cpu_data(),
            cos_theta_.mutable_cpu_data());
  ...
  switch (type_) {
    case LargeMarginInnerProductParameter_LargeMarginType_SINGLE:
      break;
    case LargeMarginInnerProductParameter_LargeMarginType_DOUBLE:
      caffe_powx(M_ * N_, cos_theta_.cpu_data(), (Dtype)2.,
                 cos_theta_quadratic_.mutable_cpu_data());
      break;
    case LargeMarginInnerProductParameter_LargeMarginType_TRIPLE:
      caffe_powx(M_ * N_, cos_theta_.cpu_data(), (Dtype)2.,
                 cos_theta_quadratic_.mutable_cpu_data());
      caffe_powx(M_ * N_, cos_theta_.cpu_data(), (Dtype)3.,
                 cos_theta_cubic_.mutable_cpu_data());
      ...
      caffe_mul(M_ * N_, sign_0_.cpu_data(), sign_2_.cpu_data(),
                sign_2_.mutable_cpu_data());
    ...
    case LargeMarginInnerProductParameter_LargeMarginType_QUADRUPLE:
      ...
      caffe_powx(M_ * N_, cos_theta_.cpu_data(), (Dtype)3.,
                 cos_theta_cubic_.mutable_cpu_data());
      caffe_powx(M_ * N_, cos_theta_.cpu_data(), (Dtype)4.,
                 cos_theta_quartic_.mutable_cpu_data());
      caffe_copy(M_ * N_, cos_theta_quadratic_.cpu_data(),
                 sign_3_.mutable_cpu_data());
      ...
      caffe_mul(M_ * N_, sign_0_.cpu_data(), sign_3_.cpu_data(),
                sign_3_.mutable_cpu_data());
      caffe_copy(M_ * N_, sign_0_.cpu_data(), sign_4_.mutable_cpu_data());
      caffe_scal(M_ * N_, (Dtype)2., sign_4_.mutable_cpu_data());
      caffe_add(M_ * N_, sign_4_.cpu_data(), sign_3_.cpu_data(),
                sign_4_.mutable_cpu_data());
  ...
  switch (type_) {
    ...
    case LargeMarginInnerProductParameter_LargeMarginType_DOUBLE: {
      ...
      for (int i = 0; i < M_; i++) {
        ...
        top_data[i * N_ + label_value] =
            xw_norm_product_data[i * N_ + label_value] *
    ...
    default: { LOG(FATAL) << "Unknown L-Softmax type."; }
  }
}

template <typename Dtype>
void LargeMarginInnerProductLayer<Dtype>::Backward_cpu(
  ...
  caffe_div(N_, inv_w_norm_.cpu_data(), w_norm_.cpu_data(),
            inv_w_norm_.mutable_cpu_data());
  caffe_cpu_gemm<Dtype>(CblasTrans, CblasNoTrans, M_, N_, 1, (Dtype)1.,
                        x_norm_.cpu_data(), inv_w_norm_.cpu_data(), (Dtype)0.,
                        xw_norm_ratio_.mutable_cpu_data());
    ...
    switch (type_) {
      ...
      case LargeMarginInnerProductParameter_LargeMarginType_DOUBLE: {
        ...
        caffe_cpu_gemm<Dtype>(CblasTrans, CblasNoTrans, N_, K_, M_,
                              lambda_ / ((Dtype)1. + lambda_), top_diff,
                              bottom_data, (Dtype)1.,
                              this->blobs_[0]->mutable_cpu_diff());
        break;
      }
                  ...
                  (Dtype)1. / ((Dtype)1. + lambda_) * top_diff[j * N_ + i] *
                      ...
                      (sign_3_data[j * N_ + i] *
                           ((Dtype)24. * cos_theta_quartic_data[j * N_ + i] -
                            (Dtype)8. * cos_theta_quadratic_data[j * N_ + i] -
                            (Dtype)1.) -
                       sign_4_data[j * N_ + i]),
                  weight + i * K_, (Dtype)1., weight_diff + i * K_);
            }
    ...
    switch (type_) {
      ...
      case LargeMarginInnerProductParameter_LargeMarginType_DOUBLE: {
        ...
        caffe_cpu_gemm<Dtype>(CblasNoTrans, CblasNoTrans, M_, K_, N_,
                              lambda_ / ((Dtype)1. + lambda_), top_diff,
                              this->blobs_[0]->cpu_data(), (Dtype)1.,
                              bottom[0]->mutable_cpu_diff());
        break;
      }
      case LargeMarginInnerProductParameter_LargeMarginType_TRIPLE: {
                  ...
                  (Dtype)1. / ((Dtype)1. + lambda_) * top_diff[i * N_ + j] /
                      ...
                      (sign_3_data[i * N_ + j] *
                           ((Dtype)24. * cos_theta_quartic_data[i * N_ + j] -
                            (Dtype)8. * cos_theta_quadratic_data[i * N_ + j] -
                            (Dtype)1.) -
                       sign_4_data[i * N_ + j]),
                  bottom_data + i * K_, (Dtype)1., bottom_diff + i * K_);
            }
