...
CAFFE_DEFINE_int32(img_size, 12, "Image Size");

namespace caffe {
void MtcnnDataGenerate(const string &input_file_name) {
  std::ifstream input_file(input_file_name, std::ios::in);
}
bool ReadImageToMTCNNDatum(const string &filename, const vector<float> &bbox,
...
void convert_data(const string &input_file_name, const string &output_folder,
                  const string &root_folder, const string &db_backend) {
  //打开文件
  ...
  std::ifstream input_file(root_folder + "/" + img_size + "/" + input_file_name,
                           std::ios::in);
  ...
  shared_ptr<caffe::db::DB> db(caffe::db::GetDB(db_backend)); //新建一个数据库
  db->Open(output_folder + "/mtcnn_train_" + db_backend, caffe::db::NEW);
  shared_ptr<caffe::db::Transaction> txn(db->NewTransaction());
...
int main(int argc, char **argv) {
  ...
  caffe::SetUsageMessage(
      "This script converts the cifar-10 dataset to\n"
      "the lmdb format used by Caffe to load data.\n"
      "Usage:\n"
      "    mtcnn_data_convert [FLAGS] input_file output_folder root_folder\n");

  caffe::ParseCommandLineFlags(&argc, &argv); //解析命令行的数据

  const string &db_backend = FLAGS_backend;

  if (argc != 4) {
    caffe::ShowUsageWithFlagsRestrict(argv[0], "cifar-10_data");
  } else {
