...
__global__ void ShuffleChannelKernel(const int nthreads,
  ...
  CUDA_KERNEL_LOOP(index, nthreads) {
    ...
    const Dtype* p_i =
        input + n * feature_map_size + (i * group_column + j) * len;
    Dtype* p_o = output + n * feature_map_size + (j * group_row + i) * len;

    for (int k = 0; k < len; k++) p_o[k] = p_i[k];
  }
}
...
void ShuffleChannelLayer<Dtype>::Forward_gpu(const vector<Blob<Dtype>*>& bottom,
  ...
  ShuffleChannelKernel<
      ...
      count, feature_map_size, top_data, bottom_data, group_row, group_column,
      sp_sz);
...
void ShuffleChannelLayer<Dtype>::Backward_gpu(
  ...
  if (propagate_down[0]) {
    ...
    ShuffleChannelKernel<
        Dtype><<<CAFFE_GET_BLOCKS(count), CAFFE_CUDA_NUM_THREADS>>>(
        count, feature_map_size, bottom_diff, top_diff, group_row, group_column,
        sp_sz);
