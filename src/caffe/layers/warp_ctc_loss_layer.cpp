...
template <typename Dtype>
void WarpCTCLossLayer<Dtype>::LayerSetUp(const vector<Blob<Dtype>*>& bottom,
...
template <typename Dtype>
void WarpCTCLossLayer<Dtype>::Reshape(const vector<Blob<Dtype>*>& bottom,
                                      const vector<Blob<Dtype>*>& top) {
...
void WarpCTCLossLayer<Dtype>::Forward_cpu(const vector<Blob<Dtype>*>& bottom,
  ...
  flat_labels_.clear();
  if (bottom.size() == 2) {  // bottom[0]=activations, bottom[1] is labels,
                             // shape: Batchsize*seq len
    ...
    for (int n = 0; n < N_; ++n) {
      ...
      for (int l = 0; l < label_len_per_batch; ++l) {
        ...
        if (label == blank_index_) continue;
    ...
    ExtractInputData(bottom[1], bottom[2], &flat_labels_, &label_lengths_,
                     &input_lengths_);
  } else if (bottom.size() == 4) {
    const Blob<Dtype>* seq_len_blob = bottom[1];
    const Blob<Dtype>* lab_len_blob = bottom[2];
  ...
  status = compute_ctc_loss_cpu(activations, gradients, flat_labels_.data(),
  ...
  // LOG(INFO) << "mean ctc loss=" << loss << ",N_="<<N_<<",num="<<num << ",
  // mean gradients="<<sumg/gcnt;
}
...
void WarpCTCLossLayer<Dtype>::Backward_cpu(const vector<Blob<Dtype>*>& top,
  ...
  CHECK_EQ(propagate_down[0], true) << "Required to propagate to probabilities";
