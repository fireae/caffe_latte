...
int main(int argc, char* argv[]) {
    ...
    for (int i = 0; i < face_infos.size(); i++) {
        cv::rectangle(show_image, face_infos[i].face_bbox, cv::Scalar(255,0,0), 1);
        for (int j = 0; j < face_infos[i].face_points.size(); j++) {
            cv::circle(show_image, face_infos[i].face_points[j], 1, cv::Scalar(255,0, 255), 1);
        }
    }
    ...
    for (int i = 0; i < face_infos.size(); i++) {
        ...
        face_bbox.x -= x;
        face_bbox.y -= y;
        cv::Rect crop_box = cv::Rect(std::max(0, face_bbox.x), std::max(0, face_bbox.y), max_span, max_span);
