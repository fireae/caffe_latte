  ...
  switch (Caffe::mode()) {
    ...
    case Caffe::GPU: {
      ...
      adagrad_update_gpu(net_params[param_id]->count(),
                         ...
                         this->history_[param_id]->mutable_gpu_data(), delta,
                         local_rate);
#else
      NO_GPU;
#endif
...
CAFFE_REGISTER_CLASS(SolverFloatRegistry, AdaGrad, AdaGradSolver<float>);
CAFFE_REGISTER_CLASS(SolverDoubleRegistry, AdaGrad, AdaGradSolver<double>);
