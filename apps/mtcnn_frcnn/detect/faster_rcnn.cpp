...
#define FRCNN_MAX(a, b) (((a) > (b)) ? (a) : (b))
#define FRCNN_MIN(a, b) (((a) < (b)) ? (a) : (b))
...
FRCNNDetector::FRCNNDetector(const string& model_dir) {
  ...
  net_ =
      boost::shared_ptr<Net<float> >(new Net<float>(model_file, caffe::TEST));
  net_->CopyTrainedLayersFrom(weights_file);
}

// load modelfile and weights
FRCNNDetector::FRCNNDetector(const string& model_file,
...
void FRCNNDetector::Detect(string im_name) {
  ...
  int max_side = FRCNN_MAX(cv_img.rows, cv_img.cols);
  int min_side = FRCNN_MIN(cv_img.rows, cv_img.cols);

  float max_side_scale = float(max_side) / float(max_input_side);
  float min_side_scale = float(min_side) / float(min_input_side);
  float max_scale = FRCNN_MAX(max_side_scale, min_side_scale);
  ...
  clock_t t1 = clock();
  net_->ForwardFrom(0);
  std::cout << " Time Using GPU-CUDNN: "
            << (clock() - t1) * 1.0 / CLOCKS_PER_SEC << std::endl;
...
void FRCNNDetector::Detect(const cv::Mat& cv_img,
                           vector<FrcnnFaceInfo>& faceinfos) {
  const int max_input_side = 500;
  const int min_input_side = 300;
  cv::Mat cv_new(cv_img.rows, cv_img.cols, CV_32FC3, cv::Scalar(0, 0, 0));
  if (cv_img.empty()) {
  ...
  for (int i = 1; i < CLASS_NUM; i++) {
    ...
    for (int k = 0; k < confidence.size(); k++) {
      if (confidence[k] > CONF_THRESH) {
        ...
        faceinfo.x1 = pred_boxes[k][0];
        faceinfo.y1 = pred_boxes[k][1];
        faceinfo.x2 = pred_boxes[k][2];
        faceinfo.y2 = pred_boxes[k][3];
        faceinfo.score = confidence[k];
        faceinfo.type = i;
...
void FRCNNDetector::Detect_video(string video_source) {
  ...
  if (!cap_.isOpened()) {
    ...
    cv::VideoWriter write(outFlie, CV_FOURCC('M', 'J', 'P', 'G'), fps,
    ...
    while (true) {
      ...
      for (int h = 0; h < cv_resized.rows; ++h) {
        for (int w = 0; w < cv_resized.cols; ++w) {
          cv_new.at<cv::Vec3f>(cv::Point(w, h))[0] =
              float(cv_resized.at<cv::Vec3b>(cv::Point(w, h))[0]) -
              float(102.9801);
          cv_new.at<cv::Vec3f>(cv::Point(w, h))[1] =
              float(cv_resized.at<cv::Vec3b>(cv::Point(w, h))[1]) -
              float(115.9465);
          cv_new.at<cv::Vec3f>(cv::Point(w, h))[2] =
              float(cv_resized.at<cv::Vec3b>(cv::Point(w, h))[2]) -
              float(122.7717);
        }
      }
      ...
      for (int h = 0; h < height; ++h) {
        for (int w = 0; w < width; ++w) {
          data_buf[(0 * height + h) * width + w] =
              float(cv_new.at<cv::Vec3f>(cv::Point(w, h))[0]);
          data_buf[(1 * height + h) * width + w] =
              float(cv_new.at<cv::Vec3f>(cv::Point(w, h))[1]);
          data_buf[(2 * height + h) * width + w] =
              float(cv_new.at<cv::Vec3f>(cv::Point(w, h))[2]);
        }
      }
      ...
      clock_t t1 = clock();
      net_->ForwardFrom(0);
      std::cout << " Forward Time Using GPU-CUDNN: "
                << (clock() - t1) * 1.0 / CLOCKS_PER_SEC << std::endl;
      bbox_delt = net_->blob_by_name("bbox_pred")->cpu_data();
      num = net_->blob_by_name("rois")->num();
...
* =====================================================================================
*/
void FRCNNDetector::vis_detections(cv::Mat image,
                                   vector<vector<float> > pred_boxes,
                                   vector<float> confidence,
                                   float CONF_THRESH) {
  for (int i = 0; i < confidence.size(); i++) {
    if (confidence[i] > CONF_THRESH) {
      cv::rectangle(image, cv::Point(pred_boxes[i][0], pred_boxes[i][1]),
...
void FRCNNDetector::boxes_sort(const int num, const float* pred,
                               float* sorted_pred) {
  vector<Info> my;
  Info tmp;
  for (int i = 0; i < num; i++) {
  ...
  for (int i = 0; i < num; i++) {
    for (int j = 0; j < 5; j++) sorted_pred[i * 5 + j] = my[i].head[j];
  }
}
...
* =====================================================================================
*/
void FRCNNDetector::bbox_transform_inv(int num, const float* box_deltas,
                                       const float* pred_cls, float* boxes,
                                       float* pred, int img_height,
                                       int img_width) {
  ...
  for (int i = 0; i < num; i++) {
    ...
    for (int j = 1; j < CLASS_NUM; j++) {
      ...
      pred[(j * num + i) * 5 + 0] =
          FRCNN_MAX(FRCNN_MIN(pred_ctr_x - 0.5 * pred_w, img_width - 1), 0);
      pred[(j * num + i) * 5 + 1] =
          FRCNN_MAX(FRCNN_MIN(pred_ctr_y - 0.5 * pred_h, img_height - 1), 0);
      pred[(j * num + i) * 5 + 2] =
          FRCNN_MAX(FRCNN_MIN(pred_ctr_x + 0.5 * pred_w, img_width - 1), 0);
      pred[(j * num + i) * 5 + 3] =
          FRCNN_MAX(FRCNN_MIN(pred_ctr_y + 0.5 * pred_h, img_height - 1), 0);
...
void FRCNNDetector::apply_nms(vector<vector<float> >& pred_boxes,
                              vector<float>& confidence) {
  for (int i = 0; i < pred_boxes.size() - 1; i++) {
    ...
    for (int j = i + 1; j < pred_boxes.size(); j++) {
      ...
      float x1 = FRCNN_MAX(pred_boxes[i][0], pred_boxes[j][0]);
      float y1 = FRCNN_MAX(pred_boxes[i][1], pred_boxes[j][1]);
      float x2 = FRCNN_MIN(pred_boxes[i][2], pred_boxes[j][2]);
      float y2 = FRCNN_MIN(pred_boxes[i][3], pred_boxes[j][3]);
