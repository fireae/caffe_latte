...
void DenseCRFLayer<Dtype>::LayerSetUp(const vector<Blob<Dtype>*>& bottom,
  ...
  CHECK_GE(bottom.size(), 2) << "bottom must have size larger than 2 (i.e., "
                                "DCNN output and image dim).";
...
void DenseCRFLayer<Dtype>::Reshape(const vector<Blob<Dtype>*>& bottom,
  ...
  //        bottom[1]: dimension for each image (i.e., store effective
  //        dimensions)
...
void DenseCRFLayer<Dtype>::Forward_cpu(const vector<Blob<Dtype>*>& bottom,
  ...
  for (int n = 0; n < num_; ++n) {
    ...
    CHECK_LE(N_, map_element_) << "The pre-allocated memory is not enough!";
...
template <typename Dtype>
void DenseCRFLayer<Dtype>::ExpAndNormalize(float* out, const float* in,
                                           float scale) {
  ...
  for (int i = 0; i < N_; ++i) {
    ...
    for (int j = 0; j < M_; ++j) V[j] /= tt;

    float* a = out + i * M_;
    for (int j = 0; j < M_; ++j) a[j] = V[j];
  }
  delete[] V;
}
...
#ifdef SSE_DENSE_CRF
  for (int i = 0; i < (N_ * M_ - 1) / 4 + 1; ++i) sse_next_[i] = -sse_unary_[i];
#else
  for (int i = 0; i < N_ * M_; ++i) next_[i] = -unary_[i];
#endif
...
template <typename Dtype>
void DenseCRFLayer<Dtype>::SetupPairwiseFunctions(
    const vector<Blob<Dtype>*>& bottom) {
  ClearPairwiseFunctions();
  ...
  if (has_image) {
    ...
    for (size_t k = 0; k < bi_w_.size(); ++k) {
      ...
      // Note H_ and W_ are the effective dimension of image (not padded
      // dimensions)
      for (int j = 0; j < H_; j++) {
        for (int i = 0; i < W_; i++) {
          ...
          // and assume im is proprocessing by scale = 1 (may cause problem if
          // not 1)
          features[(j * W_ + i) * 5 + 2] = im[img_index] / bi_rgb_std_[k];
          features[(j * W_ + i) * 5 + 3] =
              im[img_index + channel_offset] / bi_rgb_std_[k];
          features[(j * W_ + i) * 5 + 4] =
              im[img_index + 2 * channel_offset] / bi_rgb_std_[k];
        }
      }
