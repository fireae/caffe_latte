...
//#ifdef WIN32
//static float round(float v) { return floor(v + 0.5f); }
//#endif
...
class HashTable {
 ...
 public:
  ...
  int size() const { return (int)filled_; }
  void reset() {
    filled_ = 0;
    memset(table_, -1, capacity_ * sizeof(int));
  }
...
#ifdef SSE_PERMUTOHEDRAL
void Permutohedral::compute(__m128* out, const __m128* in, int value_size,
  ...
  __m128* values =
      (__m128*)_mm_malloc((M_ + 2) * value_size * sizeof(__m128), 16);
  __m128* new_values =
      (__m128*)_mm_malloc((M_ + 2) * value_size * sizeof(__m128), 16);
  ...
  for (int i = 0; i < out_size; i++) {
    ...
    for (int k = 0; k < value_size; k++) sse_val[k] = Zero;
    for (int j = 0; j <= d_; j++) {
      int o = offset_[(out_offset + i) * (d_ + 1) + j] + 1;
      __m128 w =
          _mm_set1_ps(barycentric_[(out_offset + i) * (d_ + 1) + j] * alpha);
      for (int k = 0; k < value_size; k++)
...
void Permutohedral::compute(float* out, const float* in, int value_size,
                            int in_offset, int out_offset, int in_size,
                            int out_size) const {
#ifdef SSE_PERMUTOHEDRAL
  if (in_size == -1) in_size = N_ - in_offset;
