...
void MarginInnerProductLayer<Dtype>::LayerSetUp(
  ...
  const int num_output =
      this->layer_param_.margin_inner_product_param().num_output();
  N_ = num_output;
...
template <typename Dtype>
void MarginInnerProductLayer<Dtype>::Forward_cpu(
    const vector<Blob<Dtype>*>& bottom, const vector<Blob<Dtype>*>& top) {
  iter_ += (Dtype)1.;
  ...
  Dtype lambda_min_ =
      this->layer_param_.margin_inner_product_param().lambda_min();
  ...
  for (int i = 0; i < M_; i++) {
    caffe_scal(N_, (Dtype)1. / mutable_x_norm_data[i],
               mutable_cos_theta_data + i * N_);
  }
  ...
  switch (type_) {
    ...
    case MarginInnerProductParameter_MarginType_QUADRUPLE: {
      ...
      for (int i = 0; i < M_; i++) {
        const int label_value = static_cast<int>(label[i]);
        // // |x| * (sign_3 * (8 * cos_theta_quartic - 8 * cos_theta_quadratic +
        // 1) + sign_4)
        top_data[i * N_ + label_value] =
            x_norm_data[i] *
    ...
    default: { LOG(FATAL) << "Unknown margin type."; }
  }
}

template <typename Dtype>
void MarginInnerProductLayer<Dtype>::Backward_cpu(
        ...
        for (int i = 0; i < M_; i++) {
          ...
          for (int j = 0; j < N_; j++) {
            if (label_value != j) {
              ...
              Dtype coeff_w = (Dtype)4. * sign_0_data[i * N_ + j] *
                              cos_theta_data[i * N_ + j];
              // 1 / (-|x|) * (2 * sign_0 * cos_theta_quadratic + 1) * x
              Dtype coeff_x = (Dtype)1. / (-x_norm_data[i]) *
        ...
        for (int i = 0; i < M_; i++) {
          ...
          for (int j = 0; j < N_; j++) {
            if (label_value != j) {
              ...
              // 1 / (1 + lambda) * sign_3 * (32 * cos_theta_cubic - 16 *
              // cos_theta) * w
              Dtype coeff_w = sign_3_data[i * N_ + j] *
                              ((Dtype)32. * cos_theta_cubic_data[i * N_ + j] -
                               (Dtype)16. * cos_theta_data[i * N_ + j]);
              // 1 / (-|x|) * (sign_3 * (24 * cos_theta_quartic - 8 *
              // cos_theta_quadratic - 1) +
              //                        sign_4) * x
              Dtype coeff_x =
                  (Dtype)1. / (-x_norm_data[i]) *
                  (sign_3_data[i * N_ + j] *
