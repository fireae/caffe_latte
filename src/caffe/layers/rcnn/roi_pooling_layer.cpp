    ...
    for (int c = 0; c < channels_; ++c) {
      for (int ph = 0; ph < pooled_height_; ++ph) {
        for (int pw = 0; pw < pooled_width_; ++pw) {
          ...
          int hstart =
              static_cast<int>(std::floor(static_cast<Dtype>(ph) * bin_size_h));
          int wstart =
              static_cast<int>(std::floor(static_cast<Dtype>(pw) * bin_size_w));
          int hend = static_cast<int>(
              std::ceil(static_cast<Dtype>(ph + 1) * bin_size_h));
          int wend = static_cast<int>(
              std::ceil(static_cast<Dtype>(pw + 1) * bin_size_w));
