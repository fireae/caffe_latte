...
* =====================================================================================
*/
void Detector::vis_detections(cv::Mat image, vector<vector<float> > pred_boxes,
                              vector<float> confidence, float CONF_THRESH) {
...
void Detector::boxes_sort(const int num, const float* pred,
                          float* sorted_pred) {
  vector<Info> my;
  Info tmp;
  for (int i = 0; i < num; i++) {
...
void Detector::bbox_transform_inv(int num, const float* box_deltas,
  ...
  for (int i = 0; i < num; i++) {
    ...
    for (int j = 1; j < CLASS_NUM; j++) {
      ...
      pred[(j * num + i) * 5 + 0] =
          max(min(pred_ctr_x - 0.5 * pred_w, img_width - 1), 0);
      pred[(j * num + i) * 5 + 1] =
          max(min(pred_ctr_y - 0.5 * pred_h, img_height - 1), 0);
      pred[(j * num + i) * 5 + 2] =
          max(min(pred_ctr_x + 0.5 * pred_w, img_width - 1), 0);
      pred[(j * num + i) * 5 + 3] =
          max(min(pred_ctr_y + 0.5 * pred_h, img_height - 1), 0);
...
void Detector::apply_nms(vector<vector<float> >& pred_boxes,
                         vector<float>& confidence) {
...
int main(int argc, char* argv[]) {
  ...
  string im_names =
      argv[1];  // "/home/wencc/Myplace/caffe_latte/apps/rcnn/324409_r.jpg";
  clock_t t1 = clock();
  det.Detect(im_names);
