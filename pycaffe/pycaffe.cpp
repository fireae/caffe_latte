...
PYBIND11_MODULE(pycaffe, m)
{
    ...
    py::class_<SGDSolver<Dtype>, shared_ptr<SGDSolver<Dtype>>>(m, "SGDSolver", solver)
        .def(py::init<string>());
    py::class_<NesterovSolver<Dtype>, shared_ptr<AdaGradSolver<Dtype>>>(m, "AdaGradSolver", solver)
        .def(py::init<string>());
    py::class_<AdaGradSolver<Dtype>, shared_ptr<AdaGradSolver<Dtype>>>(m, "AdaGradSolver", solver)
        .def(py::init<string>());
    py::class_<RMSPropSolver<Dtype>, shared_ptr<RMSPropSolver<Dtype>>>(m, "RMSPropSolver", solver)
        .def(py::init<string>());
    py::class_<AdaDeltaSolver<Dtype>, shared_ptr<AdaDeltaSolver<Dtype>>>(m, "AdaDeltaSolver", solver)
        .def(py::init<string>());
    py::class_<AdamSolver<Dtype>, shared_ptr<AdamSolver<Dtype>>>(m, "AdamSolver", solver)
        .def(py::init<string>());
