...
void ImageSegDataLayer<Dtype>::DataLayerSetUp(
  ...
  int crop_height = 0;
  CHECK(
      (!transform_param.has_crop_size() && transform_param.has_crop_height() &&
       transform_param.has_crop_width()) ||
  ...
  if (crop_width > 0 && crop_height > 0) {
    ...
    for (int i = 0; i < this->prefetch_.size(); ++i) {
      this->prefetch_[i]->data_.Reshape(batch_size, channels, crop_height,
                                        crop_width);
    }
    ...
    for (int i = 0; i < this->prefetch_.size(); ++i) {
      this->prefetch_[i]->label_.Reshape(batch_size, 1, crop_height,
                                         crop_width);
    }
  } else {
...
void ImageSegDataLayer<Dtype>::load_batch(Batch<Dtype>* batch) {
  ...
  for (int item_id = 0; item_id < batch_size; ++item_id) {
    ...
    if (label_type == ImageDataParameter_LabelType_PIXEL) {
      ...
      cv::Mat seg(cv_img_seg[0].rows, cv_img_seg[0].cols, CV_8UC1,
                  cv::Scalar(label));
    ...
    this->data_transformer_->TransformImgAndSeg(
        cv_img_seg, &(this->transformed_data_), &(this->transformed_label_),
        ignore_label);
