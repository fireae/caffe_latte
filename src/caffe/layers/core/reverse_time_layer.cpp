...
void ReverseTimeLayer<Dtype>::LayerSetUp(const vector<Blob<Dtype>*>& bottom,
  ...
  CHECK_EQ(bottom[1]->num_axes(), 1)
      << "Only one axis allowed for sequence length";
  CHECK_EQ(bottom[1]->count(), bottom[0]->shape(1));
}

template <typename Dtype>
void ReverseTimeLayer<Dtype>::Reshape(const vector<Blob<Dtype>*>& bottom,
                                      const vector<Blob<Dtype>*>& top) {
  top[0]->ReshapeLike(*bottom[0]);
}

template <typename Dtype>
void ReverseTimeLayer<Dtype>::Forward_cpu(const vector<Blob<Dtype>*>& bottom,
                                          const vector<Blob<Dtype>*>& top) {
  ...
  for (int n = 0; n < sub_axis_count; ++n) {
    ...
    Dtype* target =
        dest + copy_amount * sub_seq_length - copy_amount + n * sub_copy_amount;
    ...
    // 2. copy rest
    if (copy_remaining_) {
      Dtype* target = dest + (sub_seq_length)*copy_amount + n * sub_copy_amount;
