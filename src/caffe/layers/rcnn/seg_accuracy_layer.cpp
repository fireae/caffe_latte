...
void SegAccuracyLayer<Dtype>::LayerSetUp(const vector<Blob<Dtype>*>& bottom,
                                         const vector<Blob<Dtype>*>& top) {
  confusion_matrix_.clear();
  confusion_matrix_.resize(bottom[0]->channels());
  SegAccuracyParameter seg_accuracy_param =
      this->layer_param_.seg_accuracy_param();
...
template <typename Dtype>
void SegAccuracyLayer<Dtype>::Reshape(const vector<Blob<Dtype>*>& bottom,
                                      const vector<Blob<Dtype>*>& top) {
  CHECK_LE(1, bottom[0]->channels()) << "top_k must be less than or equal to "
                                        "the number of channels (classes).";
  CHECK_EQ(bottom[0]->num(), bottom[1]->num())
      << "The data and label should have the same number.";
  CHECK_EQ(bottom[1]->channels(), 1) << "The label should have one channel.";
...
void SegAccuracyLayer<Dtype>::Forward_cpu(const vector<Blob<Dtype>*>& bottom,
      ...
      for (int w = 0; w < width; ++w) {
        ...
        for (int c = 0; c < channels; ++c) {
          ...
          bottom_data_vector.push_back(
              std::make_pair(bottom_data[data_index], c));
        }
  ...
  /* for debug
  LOG(INFO) << "confusion matrix info:" << confusion_matrix_.numRows() << "," <<
  confusion_matrix_.numCols(); confusion_matrix_.printCounts();
  */
