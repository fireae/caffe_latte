...
void RoiDataLayer<Dtype>::DataLayerSetUp(const vector<Blob<Dtype> *> &bottom,
  ...
  cache_images_ = this->layer_param_.roi_data_param().cache_images();

  LOG(INFO) << "RoiDataLayer :";
  LOG(INFO) << "  source file :"
            << this->layer_param_.roi_data_param().source();
  LOG(INFO) << "  cache_images: " << (cache_images_ ? "true" : "false");
  ...
  CHECK(infile.good()) << "Failed to open roi_data file "
                       << this->layer_param_.roi_data_param().source()
                       << std::endl;
  max_long_ = this->layer_param_.roi_data_param().max_size();
...
void RoiDataLayer<Dtype>::CheckResetRois(vector<vector<float> > &rois,
  ...
  for (int i = 0; i < rois.size(); i++) {
    bool ok = rois[i][DataPrepare::X1] >= 0 && rois[i][DataPrepare::Y1] >= 0 &&
              rois[i][DataPrepare::X2] < cols &&
              rois[i][DataPrepare::Y2] < rows;
    if (ok == false) {
      DLOG(INFO) << "Roi Data Check Failed : " << image_path << " [" << i
                 << "]";
      DLOG(INFO) << " row : " << rows << ",  col : " << cols
                 << ", im_scale : " << im_scale << " | "
                 << rois[i][DataPrepare::X1] << ", " << rois[i][DataPrepare::Y1]
                 << ", " << rois[i][DataPrepare::X2] << ", "
                 << rois[i][DataPrepare::Y2];
...
void RoiDataLayer<Dtype>::load_batch(Batch<Dtype> *batch) {
  ...
  const bool mirror = this->layer_param_.roi_data_param().use_flipped();
  const int batch_size = 1;
  ...
  ShuffleImages();
  CHECK(lines_id_ < lines_.size() && lines_id_ >= 0)
      << "select error line id : " << lines_id_;
  ...
  for (int i = 1; i < channels; i++) {
    ...
    if (top_label[5 * i + 3] >= top_label[0]) {
      DLOG(INFO) << mirror << " row : " << src.rows << ",  col : " << src.cols
                 << ", im_scale : " << im_scale << " | "
                 << rois[i - 1][DataPrepare::Y2] << " , "
                 << top_label[5 * i + 3];
    }
    if (top_label[5 * i + 2] >= top_label[1]) {
      DLOG(INFO) << mirror << " row : " << src.rows << ",  col : " << src.cols
                 << ", im_scale : " << im_scale << " | "
                 << rois[i - 1][DataPrepare::X2] << " , "
                 << top_label[5 * i + 2];
