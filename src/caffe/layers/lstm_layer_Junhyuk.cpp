...
void LstmLayer<Dtype>::LayerSetUp(
  ...
  N_ = bottom[0]->shape(
      1);  // this->layer_param_.lstm_param().batch_size(); // batch_size
  H_ = this->layer_param_.lstm_param().num_output();  // number of hidden units
  I_ = bottom[0]->shape(
      2);  // bottom[0]->count() / bottom[0]->num(); // input dimension
  ...
  if (this->blobs_.size() > 0) {
    ...
    shared_ptr<Filler<Dtype> > weight_filler(
        GetFiller<Dtype>(this->layer_param_.lstm_param().weight_filler()));
    ...
    shared_ptr<Filler<Dtype> > bias_filler(
        GetFiller<Dtype>(this->layer_param_.lstm_param().bias_filler()));
...
void LstmLayer<Dtype>::Forward_cpu(const vector<Blob<Dtype>*>& bottom,
  ...
  for (int t = 0; t < T_; ++t) {
    ...
    caffe_cpu_gemm(CblasNoTrans, CblasTrans, N_, 4 * H_, H_, Dtype(1.), h_t_1,
                   weight_h, Dtype(0.), h_to_gate);
  ...
  caffe_copy(N_ * H_, cell_data + cell_.offset(T_ - 1),
...
void LstmLayer<Dtype>::Backward_cpu(const vector<Blob<Dtype>*>& top,
  ...
  for (int t = T_ - 1; t >= 0; --t) {
    ...
    Dtype* dh_t_1 =
        t > 0 ? top_diff + top_.offset(t - 1) : h_0_.mutable_cpu_diff();
    Dtype* dc_t_1 =
        t > 0 ? cell_diff + cell_.offset(t - 1) : c_0_.mutable_cpu_diff();
    const Dtype* clip_t = clip ? clip + bottom[1]->offset(t) : NULL;
    const Dtype* c_t = cell_data + cell_.offset(t);
    const Dtype* c_t_1 =
        t > 0 ? cell_data + cell_.offset(t - 1) : c_0_.cpu_data();
    ...
    for (int n = 0; n < N_; ++n) {
      ...
      for (int d = 0; d < H_; ++d) {
        ...
        pre_gate_diff_t[d] =
            gate_diff_t[d] * gate_t[d] * (Dtype(1.) - gate_t[d]);
        pre_gate_diff_t[H_ + d] =
            gate_diff_t[H_ + d] * gate_t[H_ + d] * (1 - gate_t[H_ + d]);
        pre_gate_diff_t[2 * H_ + d] = gate_diff_t[2 * H_ + d] *
                                      gate_t[2 * H_ + d] *
                                      (1 - gate_t[2 * H_ + d]);
        pre_gate_diff_t[3 * H_ + d] =
            gate_diff_t[3 * H_ + d] *
            (Dtype(1.) - gate_t[3 * H_ + d] * gate_t[3 * H_ + d]);
      }
    ...
    caffe_cpu_gemm(CblasNoTrans, CblasNoTrans, N_, H_, 4 * H_, Dtype(1.),
                   pre_gate_diff + pre_gate_.offset(t), weight_h, Dtype(0.),
                   h_to_h_.mutable_cpu_data());
    for (int n = 0; n < N_; ++n) {
      const bool cont = clip_t ? clip_t[n] : t > 0;
  ...
  if (this->param_propagate_down_[0]) {
    ...
    caffe_cpu_gemm(CblasTrans, CblasNoTrans, 4 * H_, I_, T_ * N_, Dtype(1.),
                   pre_gate_diff, bottom_data, Dtype(1.),
                   this->blobs_[0]->mutable_cpu_diff());
  }

  if (this->param_propagate_down_[1]) {
    // Gradient w.r.t. hidden-to-hidden weight
    caffe_cpu_gemm(CblasTrans, CblasNoTrans, 4 * H_, H_, (T_ - 1) * N_,
                   Dtype(1.), pre_gate_diff + pre_gate_.offset(1), top_data,
    ...
    caffe_cpu_gemm(CblasTrans, CblasNoTrans, 4 * H_, H_, 1, Dtype(1.),
                   pre_gate_diff, h_0_.cpu_data(), Dtype(1.),
  ...
  if (propagate_down[0]) {
    ...
    caffe_cpu_gemm(CblasNoTrans, CblasNoTrans, T_ * N_, I_, 4 * H_, Dtype(1.),
                   pre_gate_diff, weight_i, Dtype(0.),
