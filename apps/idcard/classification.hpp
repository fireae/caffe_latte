...
class Classifier {
 public:
  ...
  bool Init(const string& trained_file, const string& model_file,
            const string& mean_file, const string& label_file, bool gpu_mode);
  void Release() { delete this; }

  bool IsCPUMode();

  std::vector<Prediction> Classify(const string& file, int N = 5);
  std::vector<Prediction> Classify(const unsigned char* pJPGBuffer, int len,
                                   int N = 5);

  std::vector<Prediction> Classify(const cv::Mat& img, int N = 5);
  std::vector<std::vector<PredictionIdx> > BatchClassify(
      const std::vector<cv::Mat>& imgs, int N = 5);
  std::vector<Prediction> CropClassify(const cv::Mat& img, int std_size,
                                       int crop_num = 1, int N = 5);

  std::vector<PredictionIdx> ClassifyRtnIdx(const cv::Mat& img, int N = 5);

  // std::vector<float> ExtractFeature(const cv::Mat& img,const string&
  // strLayerName="");
  std::vector<float> GetLayerFeatureMaps(const string& strLayerName,
                                         std::vector<int>& outshape);
  int GetFeatureDim();
  std::vector<std::string> GetLabels() { return labels_; }
  std::vector<std::vector<float> > GetLastBlockFeature(const cv::Mat& img);
  std::vector<float> GetOutputFeatureMap(const cv::Mat& img,
                                         std::vector<int>& outshape);
  ...
  cv::Mat EstimateReceptiveField(const cv::Mat& img, const string& layerName,
                                 int x, int y, int idxNeuron = -1,
                                 bool islstm = false, int* width_parts = 0);
  void GetLayerFeatureMapSize(int w, int h, const std::string& layerName,
                              int& w1, int& h1);
