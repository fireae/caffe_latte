...
void DataTransformer<Dtype>::TransformImgAndSeg(
    ...
    Blob<Dtype>* transformed_label_blob, const int ignore_label) {
  ...
  int crop_width = 0;
  int crop_height = 0;
  CHECK((!param_.has_crop_size() && param_.has_crop_height() &&
         param_.has_crop_width()) ||
  ...
  if (pad_height > 0 || pad_width > 0) {
    cv::copyMakeBorder(
        cv_cropped_img, cv_cropped_img, 0, pad_height, 0, pad_width,
        cv::BORDER_CONSTANT,
        cv::Scalar(mean_values_[0], mean_values_[1], mean_values_[2]));
    cv::copyMakeBorder(cv_cropped_seg, cv_cropped_seg, 0, pad_height, 0,
                       pad_width, cv::BORDER_CONSTANT,
