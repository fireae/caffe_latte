...
void RankHardLossLayer<Dtype>::LayerSetUp(const vector<Blob<Dtype>*>& bottom,
  ...
  CHECK_EQ(bottom.size(), 2)
      << "RankHardLossLayer Layer takes two blobs as input.";
}
...
void RankHardLossLayer<Dtype>::set_mask(const vector<Blob<Dtype>*>& bottom) {
  ...
  for (int i = 0; i < num; i += pair_size) {
    ...
    for (int j = 0; j < num; j++) {
      ...
      Dtype tloss = max(Dtype(0), dis_data[i * num + i + 1] -
                                      dis_data[i * num + j] + Dtype(margin));
      if (tloss == 0) continue;
...
void RankHardLossLayer<Dtype>::Forward_cpu(const vector<Blob<Dtype>*>& bottom,
  ...
  for (int i = 0; i < num; i += pair_size) {
    for (int j = 0; j < num; j++) {
      ...
      Dtype tloss1 = max(Dtype(0), dis_data[i * num + i + 1] -
                                       dis_data[i * num + j] + Dtype(margin));
      Dtype tloss2 =
          max(Dtype(0), dis_data[i * num + i + 1] -
                            dis_data[(i + 1) * num + j] + Dtype(margin));
      loss += tloss1 + tloss2;
    }
  }
...
template <typename Dtype>
void RankHardLossLayer<Dtype>::Backward_cpu(
  ...
  for (int i = 0; i < count; i++) bottom_diff[i] = 0;
