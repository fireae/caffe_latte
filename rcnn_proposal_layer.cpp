...
void ProposalLayer<Dtype>::LayerSetUp(const vector<Blob<Dtype>*>& bottom,
  ...
  const RCNNProposalParameter& param = this->layer_param_.rcnn_proposal_param();
  ...
  for (int k = 0; k < anchor_num; k++) {
    ...
    for (int i = 0; i < height; i++) {
      for (int j = 0; j < width; j++) {
        if (score[anchor_num * height * width + (k * height + i) * width + j] >=
            thresh) {
          ...
          confidence.push_back(score[anchor_num * height * width +
                                     (k * height + i) * width + j]);
          tmp_bbox.push_back(bbox_deltas[(4 * k * height + i) * width + j]);
          tmp_bbox.push_back(
              bbox_deltas[((4 * k + 1) * height + i) * width + j]);
          tmp_bbox.push_back(
              bbox_deltas[((4 * k + 2) * height + i) * width + j]);
          tmp_bbox.push_back(
              bbox_deltas[((4 * k + 3) * height + i) * width + j]);
...
template <typename Dtype>
void ProposalLayer<Dtype>::_ratio_enum(vector<float> anchor,
                                       vector<float>& anchors_ratio) {
  vector<float> ctrs;
  _whctrs(anchor, ctrs);
...
template <typename Dtype>
void ProposalLayer<Dtype>::_scale_enum(vector<float> anchors_ratio,
...
template <typename Dtype>
void ProposalLayer<Dtype>::_mkanchors(vector<float> ctrs,
...
void ProposalLayer<Dtype>::bbox_transform_inv(
    int img_width, int img_height, vector<vector<float> > bbox,
    vector<vector<float> > select_anchor, vector<vector<float> >& pred) {
  int num = bbox.size();
  for (int i = 0; i < num; i++) {
    float dx = bbox[i][0];
    float dy = bbox[i][1];
...
template <typename Dtype>
void ProposalLayer<Dtype>::apply_nms(vector<vector<float> >& pred_boxes,
                                     vector<float>& confidence) {
