...
bool Classifier::Init(const string& model_path, bool gpu_mode) {
  ...
  // 		<< "Number of labels is different from the output layer
  // dimension.";
...
std::vector<Prediction> Classifier::Classify(const unsigned char* pJPGBuffer,
                                             int len, int N /*= 5*/) {
...
#endif
  if ((int)output.size() < N) N = (int)output.size();
  std::vector<int> maxN = Argmax(output, N);

  for (int i = 0; i < N; ++i) {
    int idx = maxN[i];
    predictions.push_back(std::make_pair(labels_[idx], output[idx]));
    // std::cout << "make pair:" << idx << "," << labels_[idx] << "," <<
    // output[idx] <<std::endl;
  }
...
std::vector<std::vector<PredictionIdx> > Classifier::BatchClassify(
    const std::vector<cv::Mat>& imgs, int N /*= 5*/) {
  std::vector<std::vector<PredictionIdx> > predictions;
  if (imgs.size() == 0) return predictions;
...
std::vector<Prediction> Classifier::CropClassify(const cv::Mat& img,
                                                 int std_size, int crop_num,
                                                 int N /*= 5*/) {
  ...
  if (w0 != w1 || h0 != h1) resize(img, imgresize, cv::Size(w1, h1));

  // crop
  cv::Mat imgcrop = imgresize;
  if (crop_num == 1)  // center crop
  {
    cv::Rect roi = {(w1 - input_geometry_.width) / 2 - 1,
                    (h1 - input_geometry_.height) / 2 - 1,
...
std::vector<PredictionIdx> Classifier::ClassifyRtnIdx(const cv::Mat& img,
                                                      int N /*= 5*/) {
...
std::vector<std::vector<float> > Classifier::GetLastBlockFeature(
    const cv::Mat& img) {
  Blob<float>* input_layer = net_->input_blobs()[0];
...
std::vector<float> Classifier::Predict(const cv::Mat& img) {
  Blob<float>* input_layer = net_->input_blobs()[0];
  if (input_geometry_.width != input_layer->shape(3) ||
      input_geometry_.height != input_layer->shape(2)) {
    input_layer->Reshape(1, num_channels_, input_geometry_.height,
                         input_geometry_.width);
    /* Forward dimension change to all layers. */
    net_->Reshape();
...
std::vector<std::vector<float> > Classifier::BatchPredict(
...
std::vector<float> Classifier::GetLayerFeatureMaps(const string& strLayerName,
...
// std::vector<float> Classifier::ExtractFeature(const cv::Mat& img, const
// string& strLayerName) {
// 	Blob<float>* input_layer = net_->input_blobs()[0];
...
void Classifier::Preprocess(const cv::Mat& img,
  ...
  if (!mean_.empty()) {
    ...
    cv::Mat curmean =
        cv::Mat(cv::Size(img.cols, img.rows), imgtype, channel_mean);
...
float Classifier::Pruning(float weight_t, const char* saveas_name) {
  ...
  for (size_t i = 0; i < layers.size(); i++) {
    if (layers[i]->blobs().size() == 0) continue;
    float* weights = layers[i]->blobs()[0]->mutable_cpu_data();
    int num = layers[i]->blobs()[0]->count();
...
cv::Mat Classifier::EstimateReceptiveField(const cv::Mat& img,
                                           const string& layerName, int xo,
                                           int yo, int idxNeuron, bool islstm,
                                           int* width_parts) {
  //ͨ����ȫͼ�������޸ģ���ָ����feature
  // map�ı仯�������ȷ��ָ����ָ����Ԫ�ĸо�Ұ
  ...
  if (xo < 0 || xo >= w1 || yo < 0 || yo >= h1) return cv::Mat();

  if (width_parts) *width_parts = w1;
  ...
  if (islstm) dim_feature = outshape[2];
  ...
  if (h1 > 1)  //�߶Ȳ���1
  {
    ...
    for (int i = 0; i < ny; i++) {
      ...
      for (int j = 0; j < nx; j++) {
        ...
        vRects[num].x = x0, vRects[num].y = y0, vRects[num].width = x1 - x0,
        vRects[num].height = y1 - y0;
        // modify image
        for (int y = y0; y < y1; y++) {
          for (int x = x0; x < x1; x++) {
            uchar* p = im.data + y * im.step1() + x * im.channels();
            for (int c = 0; c < ch; c++) p[c] = rand() % 256;
          }
        }
        num++;
        if (num >= batch_size || (i == ny - 1 && j == nx - 1)) {
          ...
          for (int k = 0; k < vImages.size(); k++) {
            ...
            for (int m = 0; m < channels; m++) {
              sumdiff +=
                  fabs(pf1s[m * dim + offsetxoyo] - pf0[m * dim + offsetxoyo]);
            }
            sumdiff /= channels;
    ...
    for (int x = 0; x < w; x++) {
      ...
      if (!islstm) {
        ...
        if (idxNeuron >= 0 && idxNeuron < num_feature_maps) {
          const float* pf0 =
              origResponse.data() + idxNeuron * num_feature_maps + xo;
          const float* pf1 =
              blob->cpu_data() + idxNeuron * num_feature_maps + xo;
...
void Classifier::GetLayerFeatureMapSize(int w, int h,
                                        const std::string& layerName, int& w1,
                                        int& h1) {
  Blob<float>* input_layer = net_->input_blobs()[0];
...
bool Classifier::IsCPUMode() { return (Caffe::mode() == Caffe::CPU); }
...
void Classifier::BatchForward(const vector<cv::Mat>& imgs,
                              const string& lastLayerName) {
  if (!net_->has_layer(lastLayerName)) return;
  PrepareBatchInputs(imgs);
...
void Classifier::PrepareBatchInputs(const vector<cv::Mat>& imgs) {
  ...
  if ((int)imgs.size() != input_layer->shape(0)  // image num
    ...
    input_layer->Reshape(imgs.size(), num_channels_, imgs[0].rows,
                         imgs[0].cols);
    /* Forward dimension change to all layers. */
    net_->Reshape();
  }
  ...
  for (size_t i = 0; i < imgs.size(); i++) {
    ...
    for (int j = 0; j < num_channels_; j++)
      vChannels[j].copyTo(
          input_channels
              [i * num_channels_ +
               j]);  //������copyTo����ֵ�������ڴ潻������ֵ�����޸�input_layer������
  }
}

std::vector<float> Classifier::GetOutputFeatureMap(const cv::Mat& img,
                                                   std::vector<int>& outshape) {
  PrepareInput(img);
