...
class DenseBlockLayer : public Layer<Dtype> {
 public:
  explicit DenseBlockLayer(const LayerParameter& param) : Layer<Dtype>(param) {}
  ...
  virtual void Forward_cpu_public(const vector<Blob<Dtype>*>& bottom,
                                  const vector<Blob<Dtype>*>& top);

  virtual void Backward_cpu_public(const vector<Blob<Dtype>*>& top,
                                   const vector<bool>& propagate_down,
                                   const vector<Blob<Dtype>*>& bottom);

  void Forward_gpu_public(const vector<Blob<Dtype>*>& bottom,
                          const vector<Blob<Dtype>*>& top);

  void Backward_gpu_public(const vector<Blob<Dtype>*>& top,
                           const vector<bool>& propagate_down,
                           const vector<Blob<Dtype>*>& bottom);
 ...
 protected:
  ...
  // in this->blobs_, containing all filters for Convolution, scalers and bias
  // for BN
  ...
  vector<Blob<Dtype>*> merged_conv;  // at T has shape (N,initC+T*growth,H,W),
                                     // but this vector has T+1 elements
