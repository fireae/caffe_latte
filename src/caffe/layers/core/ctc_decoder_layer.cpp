...
void CTCDecoderLayer<Dtype>::Forward_cpu(const vector<Blob<Dtype>*>& bottom,
  ...
  if (accuracy_index_ >= 0) {
    ...
    const Blob<Dtype>* target_sequences_data =
        bottom[1];  // Batchsize x labelnum
    ...
    for (int n = 0; n < N_; ++n) {
      ...
      for (int t = 0; t < labelnum; ++t) {
        ...
        if (target != blank_index_) target_sequence.push_back(target);
      }
...
void CTCGreedyDecoderLayer<Dtype>::Decode(
    const Blob<Dtype>* probabilities, const Blob<Dtype>* sequence_indicators,
...
template <typename Dtype>
void CTCGreedyDecoderLayer<Dtype>::Decode(const Blob<Dtype>* probabilities,
