...
ctcStatus_t compute_ctc_loss_gpu(const Dtype* const activations,
                                 Dtype* gradients, const int* const flat_labels,
                                 const int* const label_lengths,
                                 const int* const input_lengths,
                                 int alphabet_size, int minibatch, Dtype* costs,
                                 void* workspace, ctcOptions options) {
  ...
  if (gradients != NULL)
    return ctc.cost_and_grad(activations, gradients, costs, flat_labels,
                             label_lengths, input_lengths);
  else
    return ctc.score_forward(activations, costs, flat_labels, label_lengths,
                             input_lengths);
}
...
ctcStatus_t get_workspace_size_gpu(const int* const label_lengths,
                                   ...
                                   ctcOptions options, size_t* size_bytes) {
...
template ctcStatus_t compute_ctc_loss_gpu<float>(
    const float* const activations, float* gradients,
    const int* const flat_labels, const int* const label_lengths,
    const int* const input_lengths, int alphabet_size, int minibatch,
    float* costs, void* workspace, ctcOptions options);

template ctcStatus_t compute_ctc_loss_gpu<double>(
    const double* const activations, double* gradients,
    const int* const flat_labels, const int* const label_lengths,
    const int* const input_lengths, int alphabet_size, int minibatch,
    double* costs, void* workspace, ctcOptions);

template ctcStatus_t get_workspace_size_gpu<float>(
    const int* const label_lengths, const int* const input_lengths,
    int alphabet_size, int minibatch, ctcOptions, size_t* size_bytes);

template ctcStatus_t get_workspace_size_gpu<double>(
    const int* const label_lengths, const int* const input_lengths,
    int alphabet_size, int minibatch, ctcOptions, size_t* size_bytes);
}  // namespace ctc
