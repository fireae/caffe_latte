...
void ImageDataLayer<Dtype>::DataLayerSetUp(const vector<Blob<Dtype>*>& bottom,
  ...
  const int task_class_num =
      this->layer_param_.image_data_param().task_class_num();
  ...
  if (!is_regression) {
    ...
    for (int item_id = 0; item_id < batch_size; ++item_id) {
      ...
      for (int label_id = 0; label_id < label_size; label_id++)
        prefetch_label[item_id * label_size + label_id] =
            lines_[lines_id_].second[label_id];
    ...
    for (int item_id = 0; item_id < batch_size; ++item_id) {
      ...
      for (int value_id = 0; value_id < valuenum; value_id++)
        prefetch_label[item_id * valuenum + value_id] =
            regression_lines_[lines_id_].second[value_id];
      ...
      if (lines_id_ >= lines_size) {
        ...
        if (this->layer_param_.image_data_param().shuffle()) {
          caffe::rng_t* prefetch_rng =
              static_cast<caffe::rng_t*>(prefetch_rng_->generator());
          shuffle(regression_lines_.begin(), regression_lines_.end(),
                  prefetch_rng);
        }
      }
    }
