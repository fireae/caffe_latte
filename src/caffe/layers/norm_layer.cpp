    ...
    for (int h = 0; h < height; h++)
      for (int w = 0; w < width; w++) {
        ...
        for (int c2 = 0; c2 < channel; c2++) {
          tnum += bottom_data[i * dim + c2 * dimScale + h * width + w] *
                  top_diff[i * dim + c2 * dimScale + h * width + w] / sumdata23;
        }
        for (int c1 = 0; c1 < channel; c1++) {
          float zc1 = bottom_data[i * dim + c1 * dimScale + h * width + w];
          float tdiff = -zc1 * tnum;
          tdiff += sumdata * top_diff[i * dim + c1 * dimScale + h * width + w] /
                   sumdata23;
        ...
        /*for(int c1 = 0; c1 < channel; c1 ++)
        {
                ...
                {
                        float tnum = 0;
                        float zc1 = bottom_data[i * dim + c1 * dimScale + h *
        width + w]; float zc2 = bottom_data[i * dim + c2 * dimScale + h * width
        + w]; if(c1 == c2) tnum = (sumdata - zc1 * zc2) / sumdata23; else tnum =
        - (zc1 * zc2) / sumdata23; tnum = tnum * top_diff[i * dim + c2 *
        dimScale + h * width + w]; tdiff += tnum;
                }
                bottom_diff[i * dim + c1 * dimScale + h * width + w] = tdiff;
        }*/
      }
