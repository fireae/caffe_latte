...
bool CloseLibrary(void* handle) { return ::dlclose(handle) == 0; }
...
DynamicLibrary FindLibrary(std::string const& name,
                           std::vector<std::string> search_path) {
  for (size_t i = 0; i < search_path.size(); ++i) {
    std::string path = (filesystem::path(search_path[i]) / name).str();
